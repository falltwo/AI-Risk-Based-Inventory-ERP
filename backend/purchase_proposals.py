...
def prepare_alternative_purchase_proposal(
    *,
    ...
    with database.transaction() as conn:
        ...
        if source_event_id is not None:
            if conn.execute(
                f"SELECT 1 FROM supply_chain_events WHERE id = ? AND {valid_event_sql()}", (source_event_id,)
            ).fetchone() is None:
...
def _validate_current_proposal(
    ...
    if proposal.source_event_id is not None and conn.execute(
        f"SELECT 1 FROM supply_chain_events WHERE id = ? AND {valid_event_sql()}",

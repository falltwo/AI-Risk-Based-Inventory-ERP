...
def test_demo_purchase_orders_seed_is_opt_in_and_idempotent(l2_db, monkeypatch):
    ...
    with sqlite3.connect(l2_db) as conn:
        ...
        country, region = conn.execute("SELECT s.country,s.region FROM purchase_orders p JOIN suppliers s ON s.supplier_id=p.supplier_id ORDER BY p.po_id LIMIT 1").fetchone()
    assert risk.get_region_exposure(f"{country}|{region}")["open_po_amount"] > 0
...
def test_events_for_location_separates_news_and_formal(l2_db):
    with sqlite3.connect(l2_db) as conn:
        conn.executemany("INSERT INTO supply_chain_news(id,title,analysis_status,is_relevant,analysis_country,analysis_region,analysis_summary,estimated_delay,category) VALUES (?,?,'succeeded',1,'台灣','','fixture',?,?)", [(11,'A',7,'其他'),(12,'B',21,'氣候')])
...
NEWS = [
    {"country": "台灣", "region": "亞洲", "category": "氣候", "estimated_delay": 5,
     "title": "Typhoon", "summary": "port closed", "published_at": "2026-09-12", "id": 1, "analysis_status": "succeeded", "is_relevant": 1, "analysis_country": "台灣", "analysis_region": "亞洲", "analysis_summary": "颱風"},
]
PAYLOAD = (
    '{"摘要": "### 現況\\n台灣颱風影響港口。美國戰爭風險上升。", '
...
def test_analyze_persists_when_actor_given_and_l1_can_read(l2_db, monkeypatch):
    ...
    assert "台灣" in latest["summary"] and "美國戰爭" not in latest["summary"]
...
def test_legacy_tuple_interface_still_works(l2_db, monkeypatch):
    ...
    assert "台灣" in summary and isinstance(updates, list) and isinstance(events, list)
...
def test_scheduler_refresh_persists_summary(l2_db, monkeypatch):
    ...
    monkeypatch.setattr(risk, "batch_infer_affected_region_from_news", lambda **kw: [
        {"analysis_status": "succeeded", "is_relevant": True, "estimated_delay": 5, "event_type": "氣候",
...
def test_gate_by_evidence_drops_unsupported_and_caps_days(l2_db, monkeypatch):
    ...
    assert "台灣|亞洲" in result["evidence_locations"] and not any(k.startswith("美國|") for k in result["evidence_locations"])


def test_gate_by_evidence_unit_rules():
    evidence = risk.build_risk_evidence(
        [{"analysis_status":"succeeded", "is_relevant":1, "analysis_country":"越南", "analysis_region":"", "analysis_summary":"fixture", "category":"政策", "estimated_delay":3}],
...
def test_gate_removes_unsupported_type_when_evidence_is_unclassified():
    """證據沒有戰爭分類時，不得保留模型自行推測的戰爭類型。"""
    evidence = risk.build_risk_evidence(
        [{"analysis_status":"succeeded", "is_relevant":1, "analysis_country":"美國", "analysis_region":"北美洲", "analysis_summary":"fixture", "category":"其他", "estimated_delay":7}], [],
    )
    events = [{"event_type": "戰爭", "country": "美國", "region": "北美洲", "impact_days": 30, "description": ""}]
    _, kept, audit = risk.gate_by_evidence([], events, evidence)
    assert kept[0]["event_type"] == "其他" and kept[0]["impact_days"] == 14   # 7×2 上限
    names = {(a["name"], a["action"]) for a in audit}
    assert ("美國|北美洲", "調整") in names   # 名稱不重複國家


def test_gate_by_evidence_rejects_without_evidence():
    updates = [{"display_name": "火星", "risk_pct": 99}]
    events = [{"event_type": "戰爭", "country": "火星", "region": "", "impact_days": 90}]
    kept_u, kept_e, audit = risk.gate_by_evidence(updates, events, risk.build_risk_evidence([], []))
    assert kept_u == kept_e == [] and len(audit) == 2

...
def _find_suggestion(display_name: str, suggested_events) -> dict | None:
    """AI 建議事件裡是否有對應此據點的（沿用寬鬆的包含比對）。"""
    target = (display_name or "").strip().lower()
    for sev in suggested_events or []:
        s_r = (sev.get("region") or "").strip().lower()
        s_c = (sev.get("country") or "").strip().lower()
        if (s_r and s_r in target) or (s_c and s_c in target) or (target and (target in s_r or target in s_c)):
            return sev
    return None
            ...
            if btn_state == "view":
                ...
                if st.button("⚡ 啟動 AI 建議應變", key=f"{key}_heat_ana_ready_{i}_{reg_display}", use_container_width=True, type="primary"):
                    ...
                    add_risk_event(
                        ...
                        match_suggest.get("impact_days", 7),
                ...
                top = news_events[0]
                top_days = int(top.get("impact_days") or 0) or 7
                ...
                if st.button("🏗️ 加入應變計畫", key=f"{key}_heat_ana_manual_{i}_{reg_display}", use_container_width=True):
                    st.session_state["selected_region_for_response"] = reg_display
                    add_risk_event(
                        "其他", ev_region, ev_country, 7, f"手動加入：偵測到 {reg_display} 高風險。", actor=actor
                    )
                    st.session_state["heatmap_needs_refresh"] = True
                    st.rerun()
...
def render_supply_chain_map(
    ...
    *,
    ...
    news_items = []
    try:
        news_items = get_news_from_db(limit=10, order_by_latest=True, within_days=30) or []
    except Exception:
        pass
    ...
    if "heatmap_ai_summary" in st.session_state:
        ...
        if heatmap_rows_for_update:
            ...
            for row in heatmap_rows_for_update:
                ...
                suggested_days = 7
                sev = _find_suggestion(node_name, s_events)
                if sev:
                ...
                if risk_val is not None:
                    table_rows.append({
                        "套用": True,
                        "地區": node_name,
                        "目前 (%)": float(row.get("risk_pct") or 0),
                        "預估風險 (%)": float(risk_val),
            ...
            if table_rows:
                ...
                edited_risk_df = st.data_editor(
                    ...
                    column_config={
                        ...
                        "目前 (%)": st.column_config.NumberColumn("目前 %", disabled=True),
                        "預估風險 (%)": st.column_config.NumberColumn("AI 建議 %", min_value=0, max_value=100, step=1),
                ...
                if st.button(f"📥 套用打勾的 {len(sel_risks)} 個地區風險至地圖", key="apply_ai_risk_btn", type="primary", disabled=len(sel_risks)==0):
                    ...
                    for _, edited_row in sel_risks.iterrows():
                        sev = _find_suggestion(edited_row["地區"], current_suggested)
                        if sev:
                            sev["impact_days"] = int(edited_row["預估延遲 (天)"])

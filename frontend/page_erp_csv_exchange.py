...
def render() -> None:
    ...
    if notice:
        st.success(notice)
    current_actor = st.session_state.get("username", "")
    ...
    import_tab, export_tab, receipt_tab = st.tabs(
        ["匯入與風險預覽", "核准後匯出", "ERP 回執對帳"]
    )
    ...
    with import_tab:
        ...
        if parsed_rows is not None:
            if st.button(
                "寫入暫存區",
                type="primary",
                key="erp_csv_stage_batch",
            ):
                try:
                    summary = stage_purchase_order_rows(source_system, parsed_rows)
                    st.success(
                        "暫存完成："
        ...
        records: list[dict] = []
        try:
            records = list_exchange_records(source_system)
        except ValueError as exc:
            st.warning(f"來源識別碼無法使用：{exc}")
        except sqlite3.Error:
        ...
        if not records:
            ...
            st.dataframe(
                pd.DataFrame(record_rows), use_container_width=True, hide_index=True
            )
                    ...
                    st.write(
                        {
                            ...
                            or "未設定",
                            "供應商風險": record.get("supplier_risk_level")
                            or "未設定",
                        }
                    )
                    if state == "staged":
                        ...
                        st.success("已核准，可到「核准後匯出」下載動作檔；仍待 ERP 回執。")
    ...
    with export_tab:
        st.markdown("#### 核准後動作檔")
        st.caption(
            "只有已核准且已產生本機執行收據的最新版本會被匯出。"
            "下載不代表外部 ERP 已接收；收到回執前狀態仍是待確認。"
        )
        try:
            export_records = list_exchange_records(source_system)
            export_rows = [
                {
            ...
            action_csv = export_approved_actions_csv(
    ...
    with receipt_tab:
        ...
        try:
            st.download_button(
                "下載待填寫 ERP 回執範本",
                data=build_receipt_template_csv(
                    source_system, actor=current_actor
                ),
        ...
        try:
            receipt_records = list_exchange_receipts(
                source_system, actor=current_actor
            )
            acknowledged = [
                {
